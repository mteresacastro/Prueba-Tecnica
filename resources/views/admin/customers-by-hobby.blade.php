...
<main class="flex flex-col w-full justify-center items-center gap-4 p-4">
    ...
    <form action="{{ route('admin.customers-by-hobby') }}" method="GET">
        <label class="text-white" for="hobby">Selecciona el Hobby:</label>
    ...
    <section>
        ...
        <h2>Clientes con el hobby {{ $selectedHobby }}</h2>
            ...
            <div class="max-w-3xl px-4 py-2 space-y-4 bg-white rounded shadow dark:bg-slate-800">
                <ul class="text-xl text-slate-600 dark:text-slate-300 hover:underline">
                    <li class="flex justify-content-center gap-3">
                        ...
                        <a title="ver detalles" class="ml-4 text-white" href="{{ route('admin.show', $customer->id) }}">{{$customer->name.' '.$customer->surname}}</a>
                    </li>
                </ul>
            </div>
        @endforeach
        @else
            <p class="text-red-500">No hay clientes con el hobby seleccionado.</p>
        @endif
</section>
</main>
