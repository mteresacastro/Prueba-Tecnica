...
<main class="flex flex-col w-full gap-4 px-4 sm:grid-cols-2 md:grid-cols-3">
    ...
    <div class="flex justify-content-center">
        <a href="{{route('login')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 m-2 rounded">
            Editar cliente
        </a>
        <a href="{{route('login')}}" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 m-2 rounded">
            Borrar cliente
        </a>
    </div>
    <div class="flex align-items-center max-w-3xl px-4 py-2 space-y-4 bg-white rounded shadow text-white dark:bg-slate-800 gap-4">
        <div>
        ...
        <div>
            <h2 class="text-blue-300">Nombre: {{$customer->name.' '.$customer->surname}}</h2>
            <h2>Email: {{$customer->user->email}}</h2>
            <h3>Hobbies:</h3>
            <ul>
