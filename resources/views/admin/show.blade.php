...
<header class="px-6 py-4 space-y-2">
    <div class="flex justify-between">
        <a class="dark:text-white text-black hover:underline" href="{{route('admin.dashboard')}}">Volver</a>
        <form action="{{route('logout')}}" method="POST">
            @csrf
            <button class="dark:text-white text-black hover:underline">Cerrar sesión</button>
        </form>
    </div>
...
<main class="flex flex-col w-full justify-center items-center gap-4 p-4">
    ...
    <div class="max-w-full flex p-4 items-center justify-center bg-white rounded shadow dark:text-white text-black dark:bg-slate-800 gap-4">
